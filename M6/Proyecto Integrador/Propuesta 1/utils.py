...
def graficar(df):
    columnas = ['DIABETES', 'HOSPITALIZACIÓN ULTIMO MES', 'PSA',
    ...
    for columna in columnas:
        ...
        # Si la columna es numérica, usa un histograma
        if np.issubdtype(df[columna].dtype, np.number):
            df[columna].plot(kind='hist', rwidth=0.8)
            plt.ylabel('Frecuencia')
        # Si la columna es categórica, usa un gráfico de barras
        else:
...
def graficar1(df):
    ...
    for columna in columnas:
        ...
        # Si la columna es numérica, usa un gráfico de línea con puntos en los extremos
        if np.issubdtype(df[columna].dtype, np.number):
            sorted_df = df.sort_values(by=columna)
            plt.plot(sorted_df[columna], sorted_df['EDAD'], label=columna, color='blue')
            plt.scatter([sorted_df[columna].iloc[0], sorted_df[columna].iloc[-1]], 
                        [sorted_df['EDAD'].iloc[0], sorted_df['EDAD'].iloc[-1]], 
                        color='red')
            plt.text(sorted_df[columna].iloc[0], sorted_df['EDAD'].iloc[0], f'({sorted_df[columna].iloc[0]}, {sorted_df["EDAD"].iloc[0]})', fontsize=12)
            plt.text(sorted_df[columna].iloc[-1], sorted_df['EDAD'].iloc[-1], f'({sorted_df[columna].iloc[-1]}, {sorted_df["EDAD"].iloc[-1]})', fontsize=12)
            plt.ylabel('EDAD')
        # Si la columna es categórica, usa un gráfico de barras
        else:
            ...
            plt.axhline(max_value, color='red', linestyle='--')
            plt.text(0, max_value, f'Max: {max_value}', color='red', va='bottom', ha='right')
            plt.ylabel('Conteo')
...
def graficar_eda(df):
    for columna in df.columns:
        ...
        # Si la columna es numérica, usa un histograma
        if np.issubdtype(df[columna].dtype, np.number):
            sns.histplot(data=df, x=columna, kde=True, color='blue')
            plt.title(f'Distribución de {columna}')
            plt.ylabel('Frecuencia')
        # Si la columna es categórica, usa un gráfico de barras
        else:
            ...
            # Añade una línea horizontal en el valor máximo
            max_value = counts.max()
            plt.axhline(max_value, color='red', linestyle='--')
            # Añade una anotación con el valor máximo
            max_label = counts.idxmax()
            plt.text(max_label, max_value, f'Max: {max_value}', color='red', va='bottom', ha='right')
